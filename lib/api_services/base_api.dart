...
import 'package:demo_social_app/app_configs/environment.dart';
import 'package:demo_social_app/data_models/rest_error.dart';
...
class ApiCall {
  static Future<Response<T>> _generalApiCall<T>(
    ...
    log('URL $requestMethod $basePath/$path/$id $query ${jsonEncode(body)} ${_dio.options.headers['Authorization']}');
    ...
    try {
      if (SharedPreferenceHelper.user == null ||
        ...
        return response.data["path"] is List
            ? List<String>.from(response.data["path"].map((x) => x))
            : [response.data['path']];
      }
    } on SocketException {
      throw NoInternetError();
